...
8. **0 毫秒极速正负数字统计系统**：内存级维护标注计数缓存，支持正负净增量（`+N` / `-N`）与负数警示红框，切图快翻极致丝滑，计数稳定不跳动。
...
12. **交互式一键卸载向导**：支持“彻底删除所有文件”与“保留隔离环境卸载”双模式，彻底释放空间或保留环境随心选择。
13. **双轨极速分发与全自动环境初始化**：提供图形化安装程序（.exe）与一键自动启动脚本（.bat），首次启动全自动部署 Python/Conda 隔离环境。
