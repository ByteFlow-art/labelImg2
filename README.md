...
- **一键高速下载**：
  - **Windows 独立图形化安装向导**：[`LabelImg2_Setup_v1.0.0.exe`](https://github.com/ByteFlow-art/labelImg2/releases/download/v1.0.0/LabelImg2_Setup_v1.0.0.exe)（推荐，一键部署环境与桌面快捷方式）
  - **安装包压缩包**：[`LabelImg2_Setup_v1.0.0.zip`](https://github.com/ByteFlow-art/labelImg2/releases/download/v1.0.0/LabelImg2_Setup_v1.0.0.zip)
  - **完整源码包**：[`Source code (zip)`](https://github.com/ByteFlow-art/labelImg2/archive/refs/tags/v1.0.0.zip)

#### v1.0.0 核心功能升级与优化点
1. **内置 YOLO AI 智能模型中心**：支持 YOLOv8/v11 及自定义模型单图秒级预测与全目录多线程后台批量自动批注，画布与标签列表实时双向同步。
2. **双模式自动标注自由切换**：支持“完全覆盖替换模式”与“追加合并模式”（基于 IoU > 0.85 自动去重，完整保留已有手工标注框）。
3. **免代码闭环模型自训练控制台**：一键自动划分训练/验证集，流式实时监控 Loss / mAP50 / mAP50-95 曲线并支持权重一键导出回载。
4. **OBB 旋转框长按动态变速加速旋转**：短按 1.0° 像素级微调，长按平滑加速至 1.5°（2秒达峰值速率），松开瞬时平稳复位。
5. **OBB 主副轴尺寸独立微调**：支持 `X` 键主轴长度微调、`C` 键副轴宽度微调，尺寸微调绝不产生角度畸变。
6. **面积自适应重叠层级管理**：大框自动置底、小框优先置顶，彻底消除密集重叠目标选错与操作冲突。
7. **智能多图撤销机制 (`Ctrl+Z`)**：单图内部精准按步回退新建、删除、旋转、移动、模型标注等操作；当前图片无操作时自动返回上一张图片并回退其最后操作。
8. **0 毫秒极速内存统计系统**：内存级维护标注计数缓存，彻底消除切图磁盘 I/O 卡顿，快翻浏览丝滑流畅，标签计数平稳不跳动。
9. **负样本 0 目标安全持久化**：空图片保存时自动弹出负样本确认提示，生成标准化 0-object XML 标签文件，保障负样本训练合规。
10. **控件防误触锁定机制**：下拉选择框与微调数值框未展开时禁用滚轮滚动，防止滑动浏览界面时改乱模型超参数。
11. **独立路径解耦与状态记忆**：图片输入目录与标签输出目录完全解耦并持久化记忆，每次启动自动恢复上次真实路径。
12. **双轨极速分发与全自动环境初始化**：提供图形化安装向导（.exe）与一键自动启动脚本（.bat），首次启动全自动部署 Python/Conda 隔离环境。
...
2. 下载最新版安装程序 `LabelImg2_Setup_v1.0.0.exe`（或 `LabelImg2_Setup_v1.0.0.zip` 并解压）；
...
本项目在汲取开源社区优秀经验的基础上，进行了全方位的现代化重构与架构升级。不仅完整保留了 Pascal VOC XML 与 YOLO 格式的水平矩形框（Horizontal Box）与任意角度旋转框（Oriented Bounding Box, OBB）标注能力，更深度集成了 **YOLO 智能模型推理中心**、**单图/批量自动批注引擎**、**闭环模型自训练与流式监控** 以及 **0 毫秒极速内存统计系统**，为计算机视觉任务、遥感航拍目标检测、工业缺陷质检等场景提供了工业级的生产力加速工作流。
...
| :--- | :--- | :--- |
| **AI 自动辅助标注** | 不支持（完全依赖人工纯手工标框） | **内置 YOLO 智能模型中心**：支持单图秒级推断与全文件夹多线程后台批量自动批注，画布与标注列表实时无缝同步。 |
| **标注应用模式** | 仅支持单向覆盖写入 | **双模式自由切换**：支持“完全覆盖替换模式”与“追加合并模式”（基于 IoU > 0.85 智能去重，完好保留原图已有标注并追加新目标）。 |
| **模型闭环自训练** | 不支持（需手动切分数据、编写外部脚本训练） | **内置模型训练控制台**：免写代码，一键自动划分训练/验证集，可视化流式监控 Loss / mAP50 曲线，训练完毕即刻导出回载应用。 |
| **OBB 旋转标注体验** | 固定角度步长微调，操作繁琐易手酸 | **长按动态变速加速旋转**：短按 1.0° 像素级精调，长按平滑加速至 1.5°（2秒达峰值），松开即刻平稳复位，兼顾精准度与疾旋。 |
| **OBB 尺寸独立控制** | 仅能整体拖角缩放，易改变角度 | **主副轴长度独立缩放**：`X` 键沿主轴方向微调长度，`C` 键沿副轴方向微调宽度，尺寸微调不产生任何角度畸变。 |
| **重叠目标层级渲染** | 大小框重合时底层目标无法选取 | **面积自适应层级排序**：大框自动置底、小框优先置顶，点击拾取与边界判断精准分层，彻底消除多框重合点击误选。 |
| **撤销与回退机制** | 仅支持单图简单撤销，切图易混乱 | **智能多图撤销系统 (`Ctrl+Z`)**：单图细粒度步进撤销；当前图无操作时自动返回上一张图片并回退其最后操作。 |
| **负样本 (0目标) 保存** | 易丢失空标签文件或误抛出异常 | **标准化空标签持久化**：空图片保存时自动弹出负样本确认提示，生成标准化 0-object XML 标签文件，完美适配负样本训练。 |
| **切图响应与性能** | 切图时同步阻塞扫描磁盘，造成严重卡顿 | **0 毫秒极速内存统计系统**：内存级维护标注计数缓存，切图、长按快翻极致丝滑无延迟，总数平稳常驻绝不跳动闪烁。 |
| **会话修改状态追踪** | 列表颜色状态易在刷新重载后丢失 | **永久荧光绿会话标识**：本次运行期间新建、修改或保存的文件始终保持荧光绿高亮，存量标注文件浅灰区分，未标注透明呈现。 |
| **参数误触防篡改保护** | 鼠标滚轮滑动查看界面时易误改参数 | **控件防误触锁定机制**：下拉选择框与微调数值框未展开时自动禁用滚轮切换，防止滑动浏览界面时把模型参数改乱。 |
| **路径记忆与解耦** | 打开图片目录与保存目录易相互串扰 | **独立路径记忆机制**：图片目录与标签保存目录完全解耦并独立持久化存储，每次启动自动恢复上次真实工作路径。 |
| **软件分发与启动体验** | 仅支持命令行手动配置环境启动 | **双轨分发体系**：提供一键安装向导程序（.exe）与一键自动启动脚本（.bat），首次启动全自动配置环境，开箱即用。 |
