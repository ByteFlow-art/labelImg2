...
16. **多标签格式全兼容与无缝双向互通**：全面支持 **Pascal VOC XML (*.xml)**、**YOLO TXT (*.txt)**（含标准水平框与 8 点 OBB 旋转框）、**Create ML JSON (*.json)** 与 **COCO JSON (*.json)**。
17. **多标签格式全局持久记忆与无缝替换**：保存格式选择全局持久化记忆，每次启动自动恢复；切换格式后再次主动保存（或自动保存翻页）即可一键实现对旧格式标注的替换升级，彻底消除多格式冲突。
18. **标签组与数据集强隔离机制**：标签组统一定义管理于 `data/` 目录，杜绝在项目目录生成多余的 `classes.txt` 污染数据集；表格下拉框智能保护已有类别不被置 0。
...
| **多标签格式全兼容支持** | 仅支持 Pascal VOC XML 格式载入 | **全格式无缝双向互通**：自适应识别与加载 **Pascal VOC XML (*.xml)**、**YOLO TXT (*.txt)**（含水平框与 8 点 OBB 旋转框）、**Create ML JSON (*.json)** 与 **COCO JSON (*.json)**，支持打开已有各种格式数据集并支持多格式一键互转另存。 |
